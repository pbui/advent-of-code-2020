...
int find_triple_product(int *numbers, size_t n, int target) {
    for (size_t i = 0; i < n; i++) {
        int product = find_pair_product(numbers, n, target - numbers[i]);
        if (product) {
...
int main(int argc, char *argv[]) {
    ...
    int    product        = find_triple_product(numbers, nsize, TARGET);
